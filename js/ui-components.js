// ==============================================
// UI-KITS - Módulo de Personalización de Vehículos
// ==============================================
...
class UIKits {
  ...
  static showKitsModal(vehicleId) {
    try {
      ...
      // Mostrar modal - CORREGIDO: usar window.UIModals si existe
      if (window.UIModals && window.UIModals.showModal) {
        window.UIModals.showModal('customizationModal');
      } else {
        ...
        <div class="customization-body">
            ...
            ${kits.map(kit => `
              <div class="kit-option ${kit.id === 'standard' ? 'selected' : ''}" 
                ...
                <div class="kit-option-price">
                  ${kit.precio > 0 ? `+${this.formatPrice(kit.precio)}` : 'INCLUIDO'}
                </div>
          ...
          <div class="customization-summary" aria-live="polite">
            <div class="summary-line">
              ...
              <span class="price-value">${this.formatPrice(vehiculo.precio)}</span>
            </div>
            <div id="kit-selection-line" class="summary-line">
              ...
              <span id="selected-kit-price" class="price-value">
                ${initialKit?.precio > 0 ? `+${this.formatPrice(initialKit.precio)}` : 'INCLUIDO'}
              </span>
            </div>
            <div class="summary-total">
              <span>Total estimado</span>
              <span id="total-price" class="price-value">${this.formatPrice(initialTotal)}</span>
            </div>
  ...
  static formatPrice(price) {
    if (window.productosManager && window.productosManager.formatPrice) {
      return window.productosManager.formatPrice(price);
    }
  ...
  static setupKitsModalEvents(vehicleId) {
    ...
    document.querySelectorAll('.kit-option').forEach(option => {
      option.addEventListener('click', (e) => {
        if (e.target.closest('.kit-direct-button')) return;
    ...
    if (cancelBtn) {
      cancelBtn.addEventListener('click', () => {
        if (window.UIModals && window.UIModals.closeModal) {
          window.UIModals.closeModal('customizationModal');
        } else {
  ...
  static updateKitSummary(kit, vehicleId) {
    ...
    kitPrice.textContent = kit.precio > 0 ? `+${this.formatPrice(kit.precio)}` : 'INCLUIDO';
